...
DSP_SAMPLE _LevelToDB(const DSP_SAMPLE db)
{
  return 10.0 * log10(db);
}

DSP_SAMPLE _DBToLevel(const DSP_SAMPLE level)
{
...
DSP_SAMPLE signum(const DSP_SAMPLE val)
{
...
DSP_SAMPLE** dsp::noise_gate::Trigger::Process(DSP_SAMPLE** inputs, const size_t numChannels, const size_t numFrames)
{
  ...
  const DSP_SAMPLE alpha = pow(0.5, 1.0 / (this->mParams.GetTime() * this->mSampleRate));
  const DSP_SAMPLE beta = 1.0 - alpha;
  const DSP_SAMPLE threshold = this->mParams.GetThreshold();
  const DSP_SAMPLE dt = 1.0 / this->mSampleRate;
  const DSP_SAMPLE maxHold = this->mParams.GetHoldTime();
  const DSP_SAMPLE maxGainReduction = this->_GetMaxGainReduction();
  // Amount of open or close in a sample: rate times time
  const DSP_SAMPLE dOpen = -this->_GetMaxGainReduction() / this->mParams.GetOpenTime() * dt; // >0
  const DSP_SAMPLE dClose = this->_GetMaxGainReduction() / this->mParams.GetCloseTime() * dt; // <0
  ...
  for (auto c = 0; c < numChannels; c++)
  {
    ...
    {
      this->mLevel[c] =
        std::clamp(alpha * this->mLevel[c] + beta * (inputs[c][s] * inputs[c][s]), MINIMUM_LOUDNESS_POWER, (float)1000.0);
      const DSP_SAMPLE levelDB = _LevelToDB(this->mLevel[c]);
      ...
      { // Moving
        const DSP_SAMPLE targetGainReduction = this->_GetGainReduction(levelDB);
        if (targetGainReduction > this->mLastGainReductionDB[c])
        {
          const DSP_SAMPLE dGain = std::clamp((float)0.5 * (targetGainReduction - this->mLastGainReductionDB[c]), (float)0.0, dOpen);
        ...
        {
          const DSP_SAMPLE dGain = std::clamp((float)0.5 * (targetGainReduction - this->mLastGainReductionDB[c]), dClose, (float)0.0);
          this->mLastGainReductionDB[c] += dGain;
